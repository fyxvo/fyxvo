      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Wallet signatures stay in the browser. Fyxvo keeps only a JWT session and public wallet metadata.
            </CardDescription>
          </div>
          <Badge
            tone={
      ...
      <CardContent className="space-y-4">
        {portal.walletAddress ? (
          <div className="rounded-[1.6rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
            <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Connected wallet</div>
            ...
            <div className="mt-1 text-sm text-[var(--fyxvo-text-muted)]">
              {portal.walletName ?? "Wallet"} on <span className="text-[var(--fyxvo-text)]">{portal.walletCluster}</span>
            </div>
          </div>
        ) : null}

        {portal.networkMismatch ? (
          <Notice tone="warning" title="Wallet network mismatch" icon={<AlertIcon className="h-4 w-4" />}>
            Phantom stayed on a different cluster. Switch the wallet to Solana devnet before you prepare or send funding transactions.
          </Notice>
        ) : portal.walletPhase === "authenticated" ? (
          <Notice tone="success" title="Session ready" icon={<CheckIcon className="h-4 w-4" />}>
            Wallet ownership is verified. Project activation, funding, relay access, and analytics now share the same session boundary.
          </Notice>
        ) : null}

        {portal.errorMessage ? (
          <Notice tone="danger" title="Wallet action failed" icon={<AlertIcon className="h-4 w-4" />}>
            {portal.errorMessage}
          </Notice>
        ) : null}
