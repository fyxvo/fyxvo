  ...
  return (
    <div className="space-y-8">
      ...
      <Notice tone="neutral" title="Devnet only">
        Fyxvo is live on Solana devnet today. SOL is the public live funding path. USDC remains intentionally configuration-gated until it is explicitly enabled for the deployment.
      </Notice>

      <Notice tone="neutral" title="Need a direct line while integrating?">
        Docs cover the fastest self-serve path. For launch-fit questions, issue reports, or managed rollout conversations, use the community paths below or the contact page support forms.
        <div className="mt-4">
          <SocialLinkButtons />
        </div>
      </Notice>
      ...
      <Notice tone="neutral" title="Team and operations guide">
        The deeper project lifecycle, funding lifecycle, usage-governance expectations, managed operator posture, and private alpha onboarding notes are documented in <code>docs/team-operations.md</code> and <code>docs/private-alpha.md</code> inside the repository.
      </Notice>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        {docsSections.map((section, index) => (
          <Card key={section.title} className="fyxvo-surface border-white/5">
            <CardHeader>
              <div className="text-xs uppercase tracking-[0.18em] text-brand-300">Section {index + 1}</div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
            ...
            <CardDescription>
              These are the details most teams need when something feels ambiguous during the first devnet run.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Notice tone="neutral" title="Funding explanation">
              The API prepares the unsigned transaction. The wallet signs and sends it. The API then verifies the signature and refreshes the project’s on-chain balance view.
            </Notice>
            <Notice tone="neutral" title="Status explanation">
              The public status page reads the hosted API and gateway health surfaces and combines them with protocol readiness so the product stays honest about what is actually live.
            </Notice>
            <Notice tone="neutral" title="Monitoring and operations">
              Deployment, health surfaces, and recommended monitoring triggers are documented in <code>docs/monitoring.md</code> and <code>docs/deployment.md</code>.
            </Notice>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="fyxvo-surface border-white/5">
          <CardHeader>
            ...
            <CardDescription>
              These are the URLs most teams use first when they integrate the API and relay services.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
        ...
        <div className="space-y-6">
          <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
            <CardHeader>
              ...
              <CardDescription>
                Copy these as starting points, then replace the placeholder key with the value generated in the product.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Auth challenge</div>
                    <pre className="mt-3 overflow-x-auto text-xs leading-6 text-[var(--fyxvo-text-soft)]">
                      <code>{authExample}</code>
                    </pre>
                  </div>
                  <CopyButton value={authExample} />
                </div>
              </div>
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Standard relay request</div>
                    <pre className="mt-3 overflow-x-auto text-xs leading-6 text-[var(--fyxvo-text-soft)]">
                      <code>{standardRequest}</code>
                    </pre>
                  </div>
                  <CopyButton value={standardRequest} />
                </div>
              </div>
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Priority relay request</div>
                    <pre className="mt-3 overflow-x-auto text-xs leading-6 text-[var(--fyxvo-text-soft)]">
                      <code>{priorityRequest}</code>
                    </pre>
                  </div>
                  <CopyButton value={priorityRequest} />
                </div>
              </div>
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">TypeScript SDK</div>
                    <pre className="mt-3 overflow-x-auto text-xs leading-6 text-[var(--fyxvo-text-soft)]">
                      <code>{sdkExample}</code>
                    </pre>
                  </div>
                  <CopyButton value={sdkExample} />
                </div>
              </div>
            </CardContent>
          </Card>
          <Notice tone="neutral" title="Wallet session flow">
            The frontend requests a challenge from the API, asks the connected wallet to sign the message, then exchanges the signed payload for a JWT. That session becomes the bridge between wallet identity and project-scoped API actions.
          </Notice>
          <Notice tone="neutral" title="Funding flow">
            Funding requests are prepared by the API against the Anchor program. The browser can keep the unsigned transaction for review or send it directly through the connected wallet for a devnet confirmation path.
          </Notice>
          <Notice tone="neutral" title="Relay usage">
            Standard traffic goes to <code>/rpc</code>. Latency-sensitive traffic goes to <code>/priority</code>. Keeping these two paths separate gives teams clearer pricing and better operational discipline.
          </Notice>
          <Notice tone="neutral" title="Wallet support">
            Fyxvo supports Phantom, Solflare, Backpack, and Wallet Standard compatible wallets through the Solana wallet adapter layer. Phantom remains the most direct path for the browser-first devnet flow.
          </Notice>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
            ...
            <CardDescription>
              Project creation is not just an off-chain record. It includes the live activation transaction for the on-chain project account.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-sm leading-7 text-[var(--fyxvo-text-soft)]">
            After wallet auth, the API creates the project record, derives the PDA, and prepares the activation transaction. The project is only fully ready once the wallet signs it and the API verifies confirmation.
          </CardContent>
        </Card>
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
          ...
          <CardContent className="text-sm leading-7 text-[var(--fyxvo-text-soft)]">
            Fund your devnet wallet first, then fund the project through Fyxvo. That keeps the control plane, the Anchor program, and the gateway aligned before you scale request volume.
          </CardContent>
        </Card>
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
            ...
            <CardDescription>
              When something feels off, the shortest path to clarity is the status surface plus one direct health check.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-sm leading-7 text-[var(--fyxvo-text-soft)]">
            Check <code>{webEnv.statusPageUrl}</code>, then compare <code>{webEnv.apiBaseUrl}/health</code> and <code>{webEnv.gatewayBaseUrl}/v1/status</code>. If SOL funding confirmed but relay access still fails, check the selected project and API key before assuming a chain issue.
          </CardContent>
        </Card>
      </section>
    </div>
