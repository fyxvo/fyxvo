  ...
  const notes = [
    {
      ...
      body:
        "Wallet-authenticated project activation, SOL funding, funded gateway access, request logging, worker rollups, and analytics are operating against the live devnet program.",
      tone: "success" as const
    },
    {
      title: "USDC remains configuration-gated",
      body:
        status.apiStatus.acceptedAssets?.usdcEnabled
      ...
      tone: status.apiStatus.acceptedAssets?.usdcEnabled ? ("warning" as const) : ("neutral" as const)
    },
    {
      title: "Managed infrastructure is active",
      body:
        "The current launch topology uses Fyxvo-managed operator infrastructure. It is described as managed infrastructure, not as an open external operator marketplace.",
      tone: "neutral" as const
    },
    {
      title: "Gateway keys are scope-enforced",
      body:
        "Standard relay requires rpc:request. Priority relay requires both rpc:request and priority:relay. Under-scoped keys are rejected instead of silently receiving broad access.",
      tone: "neutral" as const
    },
    {
      ...
      tone: status.apiStatus.authorityPlan?.mode === "single-signer" ? ("warning" as const) : ("neutral" as const)
    }
  ];
      ...
      <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          ...
          <CardContent className="space-y-2">
            <Badge tone={healthy ? "success" : "warning"}>{healthy ? "healthy" : "attention needed"}</Badge>
            <p className="text-sm text-[var(--fyxvo-text-muted)]">Updated {formatRelativeDate(status.apiHealth.timestamp)}</p>
          </CardContent>
        </Card>
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
          ...
          <CardContent className="space-y-2">
            ...
            <p className="text-sm text-[var(--fyxvo-text-muted)]">Standard path average latency from the live gateway metrics surface.</p>
          </CardContent>
        </Card>
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
          ...
          <CardContent className="space-y-2">
            ...
            <p className="text-sm text-[var(--fyxvo-text-muted)]">Live success rate reported by the hosted relay.</p>
          </CardContent>
        </Card>
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
            ...
            <CardDescription>
              These values come from the live API and gateway status endpoints. They are not static copy.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 text-sm text-[var(--fyxvo-text)]">Cluster {status.apiStatus.solanaCluster}</div>
                </div>
              ...
              <div className="mt-3 text-sm text-[var(--fyxvo-text-muted)]">
                Database configured: {String(status.apiStatus.dependencies?.databaseConfigured ?? false)}. Redis configured:{" "}
                {String(status.apiStatus.dependencies?.redisConfigured ?? false)}.
              </div>
              <div className="mt-3 text-xs uppercase tracking-[0.16em]">
                <Link href={new URL("/health", webEnv.apiBaseUrl).toString()} className="text-brand-300 hover:text-brand-200">
                  API health endpoint
                </Link>
              </div>
            </div>
            ...
            <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 text-sm text-[var(--fyxvo-text)]">{status.gatewayStatus.nodeCount ?? 0} upstream nodes</div>
                </div>
              ...
              <div className="mt-3 text-sm text-[var(--fyxvo-text-muted)]">
                Scope enforcement {status.gatewayStatus.scopeEnforcement?.enabled ? "enabled" : "unavailable"}.
                Standard requires {status.gatewayStatus.scopeEnforcement?.standardRequiredScopes?.join(", ") ?? "Unavailable"}.
              </div>
              <div className="mt-3 text-xs uppercase tracking-[0.16em]">
                <Link href={new URL("/v1/status", webEnv.gatewayBaseUrl).toString()} className="text-brand-300 hover:text-brand-200">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Public links</div>
              ...
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Accepted assets</div>
                <div className="mt-2 space-y-2 text-sm text-[var(--fyxvo-text)]">
                  <p>SOL is live</p>
                  <p>USDC is {status.apiStatus.acceptedAssets?.usdcEnabled ? "enabled" : "gated"}</p>
                  <p className="break-all">{status.apiStatus.acceptedAssets?.usdcMintAddress ?? "No USDC mint configured"}</p>
                </div>
              </div>
            </div>
        ...
        <Card className="fyxvo-surface border-[color:var(--fyxvo-border)]">
          <CardHeader>
            ...
            <CardDescription>
              The wording stays explicit about what is live, what is managed, and what is intentionally still gated.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Core protocol accounts</div>
                <div className="mt-2 space-y-2 text-sm text-[var(--fyxvo-text)]">
              ...
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Managed infrastructure</div>
                <div className="mt-2 space-y-2 text-sm text-[var(--fyxvo-text)]">
              ...
              <div className="rounded-[1.5rem] border border-[color:var(--fyxvo-border)] bg-[color:var(--fyxvo-panel-soft)] p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-[var(--fyxvo-text-muted)]">Authority posture</div>
                <div className="mt-2 space-y-2 text-sm text-[var(--fyxvo-text)]">
                  <p>Mode {status.apiStatus.authorityPlan?.mode ?? "Unavailable"}</p>
                  <p className="break-all">Protocol {status.apiStatus.authorityPlan?.protocolAuthority ?? "Unavailable"}</p>
                  <p className="break-all">Pause {status.apiStatus.authorityPlan?.pauseAuthority ?? "Unavailable"}</p>
                </div>
              </div>
            </div>
            ...
            {readiness && !readiness.ready ? (
              <div className="rounded-[1.5rem] border border-amber-500/20 bg-amber-500/5 p-4">
                <div className="text-xs uppercase tracking-[0.16em] text-amber-300/80">Readiness reasons</div>
